...
public class FindIterableImpl<T> extends MongoIterableImpl<T> implements FindIterable<T> {
    private com.mongodb.reactivestreams.client.FindPublisher<T> wrapped;

    FindIterableImpl(final FindPublisher<T> wrapped) {
        super(wrapped);
    ...
    @Override
    public FindIterable<T> collation(final Collation collation) {
        wrapped.collation(collation);
        return this;
    }

    @Override
    public FindIterable<T> comment(final String comment) {
        wrapped.comment(comment);
        return this;
    }

    @Override
    public FindIterable<T> hint(final Bson hint) {
        wrapped.hint(hint);
        return this;
    }

    @Override
    public FindIterable<T> max(final Bson max) {
        wrapped.max(max);
        return this;
    }

    @Override
    public FindIterable<T> min(final Bson min) {
        wrapped.min(min);
        return this;
    }

    @Override
    public FindIterable<T> maxScan(final long maxScan) {
    ...
    @Override
    public FindIterable<T> snapshot(final boolean snapshot) {
